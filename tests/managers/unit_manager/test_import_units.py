...
def test_import_units_clones_unit(um: UnitManager):
    unit1 = Unit(Player.ONE, 6, (1, 2), reference_id = 45)

    (new_unit1,) = um.import_units((unit1,))

    assert new_unit1 is not unit1
...
def test_import_units_allows_unlinked_units(um: UnitManager):
    ...
    (new_unit1, new_unit2) = um.import_units((unit1, unit2))

    assert unit1.reference_id == 45
    assert unit2.reference_id == 46
    assert new_unit1.reference_id == next_reference_id
    assert new_unit2.reference_id == next_reference_id + 1
...
def test_import_units_allows_linked_units():
    ...
    (new_unit1, new_unit2) = um2.import_units((unit1, unit2))

    assert um1._is_linked_to_same(unit1)
    assert um2._is_not_linked_to_same(unit1)

    assert um2._is_linked_to_same(new_unit1)
    assert um2._is_linked_to_same(new_unit2)
