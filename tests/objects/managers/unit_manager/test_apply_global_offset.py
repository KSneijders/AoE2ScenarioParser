...
def test_apply_global_offset(um: UnitManager):
    ...
    um.add_units(Player.ONE, [Unit(x = i, y = i) for i in range(5)])

    um.apply_global_offset(x_offset = 1, y_offset = 0, unit_overflow_action = 'remove')
    assert len(units) == 4
    assert_units_inside_map(units, map_size, x_start_at = 1, y_start_at = 0)

    um.apply_global_offset(x_offset = 0, y_offset = 3, unit_overflow_action = 'remove')
    assert len(units) == 2
    assert_units_inside_map(units, map_size, x_start_at = 1, y_start_at = 3)

    um.apply_global_offset(x_offset = -2, y_offset = 0, unit_overflow_action = 'remove')
    assert len(units) == 1
...
def test_apply_global_offset_throw_exception(um: UnitManager):
    ...
    pytest.raises(
        ...
        lambda: um.apply_global_offset(x_offset = 1, y_offset = 1, unit_overflow_action = 'error')
    )

    pytest.raises(
        InvalidObjectPlacementError,
        lambda: um.apply_global_offset(x_offset = -3, y_offset = 0, unit_overflow_action = 'error')
    )
