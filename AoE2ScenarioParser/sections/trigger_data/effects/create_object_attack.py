...
class CreateObjectAttack(Effect):
    """
    This effect can be used to add a new attack class to a type on unit.
    """
    EFFECT_ID: int = 77

    armour_attack_quantity: int = RetrieverRef(Effect._armour_attack_quantity)
    """The amount to modify the attack class by"""

    armour_attack_class: DamageClass = RetrieverRef(Effect._armour_attack_class)
    """The attack class to modify"""

    object_id: UnitInfo | BuildingInfo | HeroInfo | OtherInfo | int = RetrieverRef(Effect._object_id)
    """The type of unit to add the attack class to"""

    source_player: Player = RetrieverRef(Effect._source_player)
    """The player whose units will have the attack class added"""

    @property
    def area(self) -> Area:
        """The area in which units will have the attack class added. When not set, units across the entire map have the attack class added"""
        return self._area
