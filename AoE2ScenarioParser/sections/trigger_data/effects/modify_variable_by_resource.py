from __future__ import annotations

from bfp_rs import RetrieverRef

from AoE2ScenarioParser.sections.trigger_data import Effect
from AoE2ScenarioParser.datasets.trigger_data import PlayerAttribute, Operation
from AoE2ScenarioParser.datasets.player_data import Player
from AoE2ScenarioParser.sections import Variable

if True:
    # ====== CUSTOM IMPORTS START ======
    pass
    # ====== CUSTOM IMPORTS END ======


class ModifyVariableByResource(Effect):
    """
    This effect can be used to modify a variable using the current value of a player's resource.
    """
    EFFECT_ID: int = 86

    __slots__ = ()
    # Keeps the memory layout identical to Effect, required for __class__ reassignment.
    # Adding new instance attributes in a subclass will break this.

    resource: PlayerAttribute | int = RetrieverRef(Effect._resource)
    """The resource whose current value will be used to modify the variable"""

    source_player: Player | int = RetrieverRef(Effect._source_player)
    """The player whose resource value will be used"""

    operation: Operation | int = RetrieverRef(Effect._operation)
    """The operation to apply to the variable using the resource value."""

    variable1_id: Variable | int = RetrieverRef(Effect._variable1_id)
    """The variable that will be modified"""

    def __init__(
        self,
        resource: PlayerAttribute | int = -1,
        source_player: Player | int = -1,
        operation: Operation | int = -1,
        variable1_id: Variable | int = -1,
    ):
        super().__init__()

        self.resource: PlayerAttribute | int = resource
        self.source_player: Player | int = source_player
        self.operation: Operation | int = operation
        self.variable1_id: Variable | int = variable1_id

    # ====== CUSTOM LOGIC START ======
    # ====== CUSTOM LOGIC END ======
