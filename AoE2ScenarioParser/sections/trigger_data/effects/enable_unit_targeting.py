...
class EnableUnitTargeting(Effect, HasSelectedUnitsAttribute, CanHoldUnits):
    """
    This effect can be used to re-enable targeting for units that were previously made untargetable.
    """
    EFFECT_ID: int = 62
    ...
    """The type of unit to enable targeting for"""

    source_player: Player | int = RetrieverRef(Effect._source_player)
    """The player whose units will have targeting re-enabled"""

    @property
    def area(self) -> Area:
        """The area in which units will have targeting enabled. When not set, units across the entire map have targeting enabled"""
        return self._area
