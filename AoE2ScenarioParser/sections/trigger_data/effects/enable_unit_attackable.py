...
class EnableUnitAttackable(Effect, HasSelectedUnitsAttribute, CanHoldUnits):
    """
    This effect can be used to re-enable attackability for units that were previously made unattackable.
    """
    EFFECT_ID: int = 99
    ...
    """The type of unit to enable attackable"""

    source_player: Player | int = RetrieverRef(Effect._source_player)
    """The player whose units will have attackable re-enabled"""

    @property
    def area(self) -> Area:
        """The area in which units will have attackable re-enabled. When not set, units across the entire map have attackable re-enabled"""
        return self._area
