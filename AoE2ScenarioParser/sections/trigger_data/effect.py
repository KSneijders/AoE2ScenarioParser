...
def selected_unit_ids():
    return [
        set_repeat(ret(Effect._selected_unit_ids)).from_(ret(Effect._properties), 4)
    ]
...
class Effect(BaseStruct):
    ...
    _selected_unit_ids: list[int]    = Retriever(i32,          default = -1, repeat = 0)
    ...
    _diplomacy: int                  = RetrieverRef(ret(_properties),  3)
    _num_objects_selected: int       = RetrieverRef(ret(_properties),  4)
    _legacy_location_object_ref: int = RetrieverRef(ret(_properties),  5)
    _unit_type1: int                 = RetrieverRef(ret(_properties),  6)
    _source_player: int              = RetrieverRef(ret(_properties),  7)
    _target_player: int              = RetrieverRef(ret(_properties),  8)
    _technology: int                 = RetrieverRef(ret(_properties),  9)
    _string_id: int                  = RetrieverRef(ret(_properties), 10)
    _sound_id: int                   = RetrieverRef(ret(_properties), 11)
    _instruction_display_time: int   = RetrieverRef(ret(_properties), 12)
    _trigger_idx: int                = RetrieverRef(ret(_properties), 13)
    ...
    _force_research_technology: int        = RetrieverRef(ret(_properties), 32)
    ...
    _unit_type2: int                       = RetrieverRef(ret(_properties), 36)
    ...
    _timer: int                            = RetrieverRef(ret(_properties), 42)
    _facet: int                            = RetrieverRef(ret(_properties), 43)
    _location_object_reference: int        = RetrieverRef(ret(_properties), 44)
    _play_sound: int                       = RetrieverRef(ret(_properties), 45)
    _color: int                            = RetrieverRef(ret(_properties), 46)
    ...
    _string_id_option1: int                = RetrieverRef(ret(_properties), 59)
    _string_id_option2: int                = RetrieverRef(ret(_properties), 60)
    ...
    _local_technology: int                 = RetrieverRef(ret(_properties), 66)
