...
class Effect(BaseStruct):
    ...
    _selected_objects: list[int]     = Retriever(i32,          default = -1, repeat = 0)  # Todo: selected_unit_ref_ids (Internally)
    ...
    _object_unit_id: int             = RetrieverRef(ret(_properties),  6)  # Todo: Change to: object_id (Internally)
    ...
    _trigger: int                    = RetrieverRef(ret(_properties), 13)  # Todo: Change to: _trigger_id (Internally)
    ...
    _flash_object: int                     = RetrieverRef(ret(_properties), 31)  # Todo: Change to: _flash_unit (Internally)
    ...
    _object_unit_id2: int                  = RetrieverRef(ret(_properties), 36)  # Todo: Change to: object2_id (Internally)
    _button_location: int                  = RetrieverRef(ret(_properties), 37)
    _ai_signal_value: int                  = RetrieverRef(ret(_properties), 38)  # Todo: Change to: _ai_signal (Internally)
    _unknown3: int                         = RetrieverRef(ret(_properties), 39)
    _object_attributes: int                = RetrieverRef(ret(_properties), 40)  # Todo: Change to: _object_attribute (Internally)
    _variable: int                         = RetrieverRef(ret(_properties), 41)  # Todo: Change to: _variable1_id (Internally)
    _timer_id: int                         = RetrieverRef(ret(_properties), 42)
    _facet: int                            = RetrieverRef(ret(_properties), 43)
    _location_object_ref: int              = RetrieverRef(ret(_properties), 44)  # Todo: Change to: _location_unit_ref (Internally)
    ...
    _object_state: int                     = RetrieverRef(ret(_properties), 50)  # Todo: Change to: _unit_state (Internally)
    ...
    _variable2: int                        = RetrieverRef(ret(_properties), 61)  # Todo: Change to: _variable2_id (Internally)
    ...
    _object_group2: int                    = RetrieverRef(ret(_properties), 68)  # Todo: Change to: _object2_group (Internally)
    _object_type2: int                     = RetrieverRef(ret(_properties), 69)  # Todo: Change to: _object2_type (Internally)
